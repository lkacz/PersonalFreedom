...
from gamification import generate_item, LUCKY_OPTION_CHANCES, NEIGHBOR_EFFECT_CHANCES, LUCK_BOOST_CHANCES
...
for rarity in rarities:
    ...
    print(f'  Neighbor Effect: {neighbor_effect_count}/10 (expected ~{neighbor_chance}%)')
    print()
...
for i in range(50):
    ...
    if item.get('luck_boost') and item.get('lucky_options') and item.get('neighbor_effect'):
        ...
        print(f'Neighbor Effect: {item["neighbor_effect"]}')
        break
else:
    print('(Could not find item with all 3 attributes in 50 tries - checking individual items)')
    ...
    print(f'  Neighbor Effect: {item.get("neighbor_effect", "None")}')
