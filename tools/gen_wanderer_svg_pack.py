...
def helmet_svg(rarity: str, idx: int) -> str:
    ...
    cresc = ""
    if idx >= 1:
        cresc = f'<path d="M82 37 Q88 30 94 37 Q88.5 38.5 82 37 Z" fill="none" stroke="{c}" stroke-width="1" opacity="0.8"/>'
    diadem = ""
    if idx >= 2:
        diadem = f'<path d="M74 43 Q90 34 106 43" stroke="{c}" stroke-width="1" fill="none" opacity="0.9"/>'
    star = ""
    if idx >= 3:
        star = f'<path d="M90 37 L91.2 40 L94.2 40.2 L92 42.2 L92.6 45 L90 43.6 L87.4 45 L88 42.2 L85.8 40.2 L88.8 40 Z" fill="{c}" opacity="0.95"/>'
    crown = ""
    if idx >= 4:
        crown = f'<path d="M72 33 L79 29 L86 33 L93 29 L100 33 L107 29 L114 33 L114 37 L72 37 Z" fill="#2d4366" stroke="{c}" stroke-width="0.9"/>'
    halo = ""
    if idx == 5:
        halo = f'<ellipse cx="90" cy="43.5" rx="29" ry="9.4" fill="none" stroke="{c}" stroke-width="1" opacity="0.56"/><ellipse cx="90" cy="43.5" rx="23.5" ry="7.2" fill="none" stroke="#bcf8ff" stroke-width="0.82" opacity="0.58"/>'
    body = f"""
  <g id="helmet_{rarity}">
    <path d="M66 44 Q90 35 114 44 L110 51 Q90 49 70 51 Z" fill="#223857" stroke="#122038" stroke-width="1"/>
    <path d="M78 42 L102 42 L102 48 L78 48 Z" fill="#324c72" stroke="#1b304d" stroke-width="0.8"/>
    <circle cx="84.8" cy="47.2" r="1.8" fill="#d6e9ff"/>
    <circle cx="95.2" cy="47.2" r="1.8" fill="#d6e9ff"/>
    <circle cx="84.8" cy="47.2" r="0.7" fill="#26303e"/>
    <circle cx="95.2" cy="47.2" r="0.7" fill="#26303e"/>
    <path d="M87.3 47.2 L92.7 47.2" stroke="#adc5eb" stroke-width="0.8"/>
    {cresc}
    {diadem}
    {star}
    {crown}
...
def chestplate_svg(rarity: str, idx: int) -> str:
    ...
    details = ""
    if idx >= 1:
        details += '<path d="M74 92 L106 92 M74 103 L106 103 M74 114 L106 114" stroke="#7991bc" stroke-width="0.85" opacity="0.72"/>'
    if idx >= 2:
        details += f'<path d="M72 82 L80 76 L100 76 L108 82" stroke="{c}" stroke-width="1" fill="none" opacity="0.82"/>'
    if idx >= 3:
        details += f'<path d="M90 94 L92.2 99 L97.5 99.4 L93.5 102.8 L94.8 108 L90 105.2 L85.2 108 L86.5 102.8 L82.5 99.4 L87.8 99 Z" fill="{c}" opacity="0.9"/>'
    if idx >= 4:
        details += f'<path d="M68 86 Q90 70 112 86" stroke="{c}" stroke-width="1.25" fill="none" opacity="0.84"/><path d="M70 130 Q90 141 110 130" stroke="{c}" stroke-width="1.1" fill="none" opacity="0.7"/>'
    if idx == 5:
        details += f'<ellipse cx="90" cy="103" rx="18" ry="7.2" fill="none" stroke="{c}" stroke-width="0.95" opacity="0.54"/><ellipse cx="90" cy="103" rx="12.5" ry="5.2" fill="none" stroke="#bcf8ff" stroke-width="0.8" opacity="0.58"/>'
    body = f"""
  <g id="chestplate_{rarity}">
    <path d="M70 79 L110 79 Q115 86 115 97 L112 145 L68 145 L65 97 Q65 86 70 79 Z" fill="#354d73" stroke="#182942" stroke-width="1.1"/>
    <path d="M76 84 L104 84 L105 143 L75 143 Z" fill="#4f6794" opacity="0.9"/>
    <path d="M76 84 L83 94 L80 143 L74 143 Z" fill="#314766" opacity="0.74"/>
    <path d="M104 84 L97 94 L100 143 L106 143 Z" fill="#314766" opacity="0.74"/>
    <path d="M90 84 L90 143" stroke="#243654" stroke-width="1"/>
    {details}
  </g>
"""
    return wrap(body)
...
def gauntlets_svg(rarity: str, idx: int) -> str:
    ...
    details = ""
    if idx >= 1:
        details += '<path d="M40 143 L60 143 M120 143 L140 143" stroke="#7a93bd" stroke-width="0.82" opacity="0.72"/>'
    if idx >= 2:
    ...
    if idx >= 4:
        details += f'<path d="M39 145.8 L61 145.8" stroke="{c}" stroke-width="1.2" opacity="0.82"/><path d="M119 145.8 L141 145.8" stroke="{c}" stroke-width="1.2" opacity="0.82"/>'
    if idx == 5:
  ...
  <g id="gauntlets_{rarity}">
    <path d="M38 134 L62 134 L61 156 L39 156 Z" fill="#385178" stroke="#1b2f4b" stroke-width="1"/>
    <path d="M118 134 L142 134 L141 156 L119 156 Z" fill="#385178" stroke="#1b2f4b" stroke-width="1"/>
    <rect x="42.8" y="150.2" width="14.3" height="2.8" rx="1.2" fill="#d8e7ff" opacity="0.92"/>
    <rect x="122.8" y="150.2" width="14.3" height="2.8" rx="1.2" fill="#d8e7ff" opacity="0.92"/>
    {details}
  </g>
"""
    return wrap(body)
...
def boots_svg(rarity: str, idx: int) -> str:
    ...
    details = ""
    if idx >= 1:
        details += '<path d="M44 194 L73 194 M106 194 L135 194" stroke="#8098c1" stroke-width="0.82" opacity="0.7"/>'
    if idx >= 2:
    ...
    if idx >= 3:
        details += f'<path d="M45 202.5 L74 202.5" stroke="{c}" stroke-width="1.05" opacity="0.84"/><path d="M106 202.5 L135 202.5" stroke="{c}" stroke-width="1.05" opacity="0.84"/>'
    if idx >= 4:
    ...
    body = f"""
  <g id="boots_{rarity}">
    <path d="M43 188 L74 188 L76 202 L41 202 Z" fill="#395276" stroke="#1c304b" stroke-width="1"/>
    <path d="M104 188 L135 188 L137 202 L102 202 Z" fill="#395276" stroke="#1c304b" stroke-width="1"/>
    <path d="M41 201 L77 201 L79 209 L39 209 Z" fill="#262f3f" stroke="#111924" stroke-width="1"/>
    <path d="M102 201 L138 201 L140 209 L100 209 Z" fill="#262f3f" stroke="#111924" stroke-width="1"/>
    <path d="M45 192 L74 192" stroke="{c}" stroke-width="1.08" opacity="0.88"/>
    <path d="M106 192 L135 192" stroke="{c}" stroke-width="1.08" opacity="0.88"/>
    {details}
  </g>
"""
    return wrap(body)
...
def shield_svg(rarity: str, idx: int) -> str:
    ...
    details = ""
    if idx >= 1:
        details += '<path d="M37 110 L59 110 M37 118 L59 118 M37 126 L59 126 M37 134 L59 134" stroke="#7f97c2" stroke-width="0.8" opacity="0.72"/>'
    if idx >= 2:
        details += f'<path d="M40 103 L58 103 L62 109 L62 140 L58 146 L40 146 L36 140 L36 109 Z" fill="none" stroke="{c}" stroke-width="0.88" opacity="0.82"/>'
    if idx >= 3:
    ...
    if idx >= 4:
        details += f'<path d="M44 126 L49 131 L55 121" stroke="{c}" stroke-width="1.05" fill="none"/><circle cx="49.5" cy="110.4" r="2.0" fill="#243757" stroke="{c}" stroke-width="0.86"/>'
    if idx == 5:
        details += f'<ellipse cx="49" cy="125" rx="17.2" ry="8.2" fill="none" stroke="{c}" stroke-width="0.92" opacity="0.55"/><ellipse cx="49" cy="125" rx="11.7" ry="5.5" fill="none" stroke="#bcf8ff" stroke-width="0.78" opacity="0.58"/>'
    body = f"""
  <g id="shield_{rarity}">
    <path d="M34 100 L60 100 L64 106 L64 142 L60 149 L34 149 L30 142 L30 106 Z" fill="#395377" stroke="#1b304c" stroke-width="1"/>
    <rect x="36.5" y="103.4" width="21" height="42" rx="2" fill="#4f6995" opacity="0.9"/>
    <path d="M30 106 L64 106" stroke="{c}" stroke-width="0.95" opacity="0.82"/>
    {details}
  </g>
"""
    return wrap(body)
...
def weapon_svg(rarity: str, idx: int) -> str:
    ...
    if idx >= 2:
        details += f'<path d="M127 111.5 Q139 106 147 99" stroke="{c}" stroke-width="0.82" fill="none" opacity="0.75"/>'
    if idx >= 3:
        details += f'<circle cx="142" cy="102.5" r="1.5" fill="{c}" opacity="0.95"/><path d="M124 124 L128 120 L131 126 L127 130 Z" fill="#2c4564" stroke="{c}" stroke-width="0.84"/>'
    if idx >= 4:
        details += f'<path d="M124 106 Q140 94 151 106" stroke="{c}" stroke-width="1.22" fill="none" opacity="0.9"/><path d="M126 118 Q140 112 149 105" stroke="{c}" stroke-width="0.86" fill="none" opacity="0.8"><animate attributeName="opacity" values="0.45;1;0.45" dur="1.5s" repeatCount="indefinite"/></path>'
    if idx == 5:
        details += f'<ellipse cx="142" cy="103" rx="10.8" ry="4.3" fill="none" stroke="{c}" stroke-width="0.84" opacity="0.55"/>'
    body = f"""
  <g id="weapon_{rarity}" transform="rotate(-24 121 131)">
    <rect x="115.2" y="126" width="10.2" height="36" rx="3" fill="#21354f" stroke="#111c2d" stroke-width="1"/>
    ...
    <rect x="118.8" y="160" width="3.2" height="4.8" rx="1.4" fill="#2e4767"/>
    <path d="M114.8 126 L125.6 126 L127 122 L113.6 122 Z" fill="#425f87" stroke="#1d304c" stroke-width="1"/>
    <path d="M124 102 Q141 92 152 106 Q138 111 126 124 Z" fill="#d9e9ff" stroke="#3f5d83" stroke-width="1"/>
...
def cloak_svg(rarity: str, idx: int) -> str:
    ...
    details = ""
    if idx >= 1:
        details += '<path d="M72 94 L68 182 M108 94 L112 182" stroke="#9db6de" stroke-width="0.8" opacity="0.45"/>'
    if idx >= 2:
  ...
  <g id="cloak_{rarity}" opacity="0.9">
    <path d="M63 74 L117 74 L124 92 L117 191 L63 191 L56 92 Z" fill="#223659" opacity="0.48"/>
    <path d="M68 82 L112 82 L116 97 L112 186 L68 186 L64 97 Z" fill="#466193" opacity="0.44"/>
    <path d="M70 150 L62 191 L74 191 Z" fill="#2b4368" opacity="0.68"/>
    <path d="M108 150 L120 189 L106 190 Z" fill="#2b4368" opacity="0.68"/>
    {details}
  </g>
"""
    return wrap(body)
...
def amulet_svg(rarity: str, idx: int) -> str:
    ...
    body = f"""
  <g id="amulet_{rarity}">
    <path d="M86 90 Q90 86 94 90" stroke="#90a7ca" stroke-width="1.02" fill="none"/>
    <circle cx="90" cy="100" r="5.1" fill="#344f78" stroke="#182940" stroke-width="1"/>
