...
STORY_GEAR_THEMES = {
    ...
    "underdog": {
        ...
        "set_themes": {
            ...
            "Coffee": {"words": ["coffee", "mug", "caffeine", "espresso"], "bonus_per_match": 15, "emoji": SYMBOL_EMOJI["coffee"]},
...
`If R-0 ever reads this — I stayed because I believed in his heart, even when he hid it.`
...
Your choices — what you obeyed, who you loved, and how you fought — decided what kind of leader you became.
""",
        "endings": {
            "AAA": {
...
You finally confess — not in crisis, but in peace.
...
Together you start a school where humans and robots learn to argue, build, and be honest — all in the same class.
...
Mara nearly dies during the blackout, but lives because workers — human and robot — shield her with their own bodies.
...
Then you speak — not as evidence, but as a citizen.
...
Your choices — what rules you followed, who you trusted, and how you fought — decided whether you became a war hero, a builder, or both.
""",
        "endings": {
            "AAA": {
...
The empire falls — not in fire, but in court records, worker votes, and signed agreements.
...
You finally answer — not in crisis, but in calm — and slowly rebuild what silence broke.
...
Lesson:
**A reckless past does not stop you from building lasting justice — if you commit to honesty going forward.**
""",
            },
            "BBA": {
...
The room votes to keep you — not because you were perfect, but because you owned your failures.
...
Then you win anyway — through evidence, public pressure, and signed law.
...
Your choices — who you were loyal to, what rules you accepted, and how you ended it — shaped who you became.
""",
        "endings": {
            "AAA": {
...
You get promoted — not for dramatic arrests, but for fixing real damage.
...
You take it — later than planned — and become known for thorough, careful work against corruption.
    ...
    if week_ago_entry and week_ago_entry["date"] != current_date:
        ...
        if weight_mode == WEIGHT_MODE_GAIN:
            ...
            if weekly_gain >= WEIGHT_WEEKLY_LEGENDARY_THRESHOLD:
                ...
                result["messages"].append(
                    f"🌟 WEEKLY {final_rarity.upper()}! Gained {weekly_gain:.0f}g this week! "
                ...
                result["messages"].append(
                    f"🏆 Great week! Gained {weekly_gain:.0f}g - {final_rarity} reward!"
                )
                if lock_message:
                    result["messages"].append(f"🔒 {lock_message}")
        elif weight_mode == WEIGHT_MODE_MAINTAIN:
            ...
            if abs_weekly <= 200:  # Very stable week
                ...
                result["messages"].append(
                    f"⚖️ Stable week! (±{abs_weekly:.0f}g) {final_rarity} reward!"
                )
                if lock_message:
                    result["messages"].append(f"🔒 {lock_message}")
            ...
            if weekly_change >= WEIGHT_WEEKLY_LEGENDARY_THRESHOLD:
                final_rarity, lock_message = _set_timed_reward("weekly", "Legendary")
                result["messages"].append(
                    f"🌟 WEEKLY {final_rarity.upper()}! Lost {weekly_change:.0f}g this week! "
                    f"(vs {week_ago_entry['date']})"
                )
                if lock_message:
                    result["messages"].append(f"🔒 {lock_message}")
                ...
                final_rarity, lock_message = _set_timed_reward("weekly", "Epic")
                result["messages"].append(
                    f"🏆 Great week! Lost {weekly_change:.0f}g - {final_rarity} reward!"
                )
                if lock_message:
                    result["messages"].append(f"🔒 {lock_message}")
    ...
    if month_ago_entry and month_ago_entry["date"] != current_date:
        ...
        if weight_mode == WEIGHT_MODE_GAIN:
            ...
            if monthly_gain >= WEIGHT_MONTHLY_LEGENDARY_THRESHOLD:
                ...
                result["messages"].append(
                    f"💒 MONTHLY {final_rarity.upper()}! Gained {monthly_gain/1000:.1f}kg this month! "
                ...
                result["messages"].append(
                    f"🎊 Amazing month! Gained {monthly_gain/1000:.1f}kg - {final_rarity} reward!"
                )
                if lock_message:
                    result["messages"].append(f"🔒 {lock_message}")
        elif weight_mode == WEIGHT_MODE_MAINTAIN:
            ...
            if abs_monthly <= 500:  # Very stable month
                final_rarity, lock_message = _set_timed_reward("monthly", "Epic")
                result["messages"].append(
                    f"⚖️ Rock-solid month! (±{abs_monthly:.0f}g) {final_rarity} reward!"
                )
                if lock_message:
                    result["messages"].append(f"🔒 {lock_message}")
                ...
                result["messages"].append(
                    f"⚖️ Good month! (±{abs_monthly:.0f}g) {final_rarity} reward!"
                )
                if lock_message:
                    result["messages"].append(f"🔒 {lock_message}")
        else:  # WEIGHT_MODE_LOSS
            if monthly_change >= WEIGHT_MONTHLY_LEGENDARY_THRESHOLD:
                final_rarity, lock_message = _set_timed_reward("monthly", "Legendary")
                result["messages"].append(
                    f"💒 MONTHLY {final_rarity.upper()}! Lost {monthly_change/1000:.1f}kg this month! "
                    f"(vs {month_ago_entry['date']})"
                )
                if lock_message:
                    result["messages"].append(f"🔒 {lock_message}")
                ...
                final_rarity, lock_message = _set_timed_reward("monthly", "Epic")
                result["messages"].append(
                    f"🎊 Amazing month! Lost {monthly_change/1000:.1f}kg - {final_rarity} reward!"
                )
                if lock_message:
                    result["messages"].append(f"🔒 {lock_message}")
    ...
    for days, rarity in sorted(WEIGHT_STREAK_THRESHOLDS.items()):
        ...
        if streak >= days and milestone_id not in achieved_milestones:
            ...
            return {
                ...
                "message": (
                    f"🔥 {days}-Day Logging Streak! Earned a {final_rarity} item!"
        ...
        try:
            if milestone["check"](weight_entries, goal, starting_weight):
                ...
                new_milestones.append({
                    ...
                    "item": generate_item(rarity=final_rarity, story_id=story_id),
                    "message": (
                        f"🏆 Milestone: {milestone['name']}! ({milestone['description']}) - {final_rarity} reward!"
                        + (f" 🔒 {lock_message}" if lock_message else "")
                    ),
                })
        except Exception:
...
def check_weight_maintenance(
    ...
    if maintenance_days >= 30:
        ...
        message = "💒 30+ Days in Maintenance! Perfect control!"
    elif maintenance_days >= 14:
        requested_rarity = "Epic"
        message = "🌟 2+ Weeks in Maintenance! Excellent stability!"
    elif maintenance_days >= 7:
        requested_rarity = "Rare"
        message = "💪 1 Week in Maintenance! Great job staying on target!"
    else:
        requested_rarity = "Uncommon"
        message = f"✅ Maintaining goal weight! ({deviation*1000:.0f}g from target)"
...
ACTIVITY_TYPES = [
    ("walking", "Walking", ACTIVITY_EMOJI["walking"], 1.0),
    ("jogging", "Jogging", ACTIVITY_EMOJI["jogging"], 1.5),
    ("running", "Running", ACTIVITY_EMOJI["running"], 2.0),
    ("cycling", "Cycling", ACTIVITY_EMOJI["cycling"], 1.5),
    ("swimming", "Swimming", ACTIVITY_EMOJI["swimming"], 2.0),
    ("hiking", "Hiking", ACTIVITY_EMOJI["hiking"], 1.5),
    ("strength", "Strength Training", ACTIVITY_EMOJI["strength"], 2.0),
    ("yoga", "Yoga", ACTIVITY_EMOJI["yoga"], 1.0),
    ("dance", "Dancing", ACTIVITY_EMOJI["dance"], 1.5),
    ("sports", "Sports (General)", ACTIVITY_EMOJI["sports"], 1.8),
    ("hiit", "HIIT", ACTIVITY_EMOJI["hiit"], 2.5),
    ("climbing", "Climbing", ACTIVITY_EMOJI["climbing"], 2.0),
    ("martial_arts", "Martial Arts", ACTIVITY_EMOJI["martial_arts"], 2.0),
    ("stretching", "Stretching", ACTIVITY_EMOJI["stretching"], 0.8),
    ("other", "Other Activity", ACTIVITY_EMOJI["other"], 1.0),
]
    ...
    for days, rarity, name in ACTIVITY_STREAK_THRESHOLDS:
        ...
        if streak >= days and milestone_id not in achieved_milestones:
            ...
            return {
                ...
                "item": generate_item(rarity=final_rarity, story_id=story_id),
                "message": (
                    f"🔥 {days}-day activity streak! Earned a {final_rarity} item!"
                    + (f" 🔒 {lock_message}" if lock_message else "")
                ),
            }
...
SLEEP_CHRONOTYPES = [
    ("early_bird", "Early Bird (Lark)", SLEEP_CHRONOTYPE_EMOJI["early_bird"], "21:00", "22:30", 
     "Naturally wake early. Optimal bedtime: 9-10:30 PM"),
    ("moderate", "Moderate", SLEEP_CHRONOTYPE_EMOJI["moderate"], "22:00", "23:30", 
     "Balanced schedule. Optimal bedtime: 10-11:30 PM"),
    ("night_owl", "Night Owl", SLEEP_CHRONOTYPE_EMOJI["night_owl"], "23:00", "00:30", 
...
SLEEP_QUALITY_FACTORS = [
    ("excellent", "Excellent", SLEEP_QUALITY_EMOJI["excellent"], 1.3),
    ("good", "Good", SLEEP_QUALITY_EMOJI["good"], 1.1),
    ("fair", "Fair", SLEEP_QUALITY_EMOJI["fair"], 1.0),
    ("poor", "Poor", SLEEP_QUALITY_EMOJI["poor"], 0.8),
    ("terrible", "Terrible", SLEEP_QUALITY_EMOJI["terrible"], 0.6),
]

# Sleep disruption tags
# (tag_id, display_name, emoji, impact)
SLEEP_DISRUPTION_TAGS = [
    ("none", "No disruptions", SLEEP_DISRUPTION_EMOJI["none"], 0),
    ("woke_once", "Woke once", SLEEP_DISRUPTION_EMOJI["woke_once"], -5),
    ("woke_multiple", "Woke multiple times", SLEEP_DISRUPTION_EMOJI["woke_multiple"], -15),
    ("nightmares", "Bad dreams", SLEEP_DISRUPTION_EMOJI["nightmares"], -10),
    ("noise", "Noise disruption", SLEEP_DISRUPTION_EMOJI["noise"], -10),
    ("stress", "Stress/anxiety", SLEEP_DISRUPTION_EMOJI["stress"], -15),
    ("late_caffeine", "Late caffeine", SLEEP_DISRUPTION_EMOJI["late_caffeine"], -10),
    ("late_screen", "Late screen time", SLEEP_DISRUPTION_EMOJI["late_screen"], -10),
    ("alcohol", "Alcohol", SLEEP_DISRUPTION_EMOJI["alcohol"], -15),
]
...
def check_water_entry_reward(
    ...
    if glasses_today < HYDRATION_MAX_DAILY_GLASSES <= new_glass_number and streak_days > 0:
        ...
        if streak_rarity:
            ...
            message = f"🔥 Hydration streak: {streak_days + 1} days! Earned a {final_rarity} item!"
            if lock_message:
                message = f"{message} 🔒 {lock_message}"
    ...
    for days, rarity, name in SLEEP_STREAK_THRESHOLDS:
        ...
        if streak >= days and milestone_id not in achieved_milestones:
            ...
            return {
                ...
                "item": generate_item(rarity=final_rarity, story_id=story_id),
                "message": (
                    f"🌙 {days}-night sleep streak! Earned a {final_rarity} item!"
                    + (f" 🔒 {lock_message}" if lock_message else "")
                ),
            }
...
# Level titles
LEVEL_TITLES = {
    1: ("Novice", LEVEL_TITLE_EMOJI[1]),
    5: ("Apprentice", LEVEL_TITLE_EMOJI[5]),
    10: ("Focused", LEVEL_TITLE_EMOJI[10]),
    15: ("Dedicated", LEVEL_TITLE_EMOJI[15]),
    20: ("Disciplined", LEVEL_TITLE_EMOJI[20]),
    25: ("Expert", LEVEL_TITLE_EMOJI[25]),
    30: ("Master", LEVEL_TITLE_EMOJI[30]),
    40: ("Grandmaster", LEVEL_TITLE_EMOJI[40]),
    50: ("Legend", LEVEL_TITLE_EMOJI[50]),
    75: ("Mythic", LEVEL_TITLE_EMOJI[75]),
    100: ("Transcendent", LEVEL_TITLE_EMOJI[100]),
}
...
def get_level_title(level: int) -> tuple:
    ...
    title = ("Novice", LEVEL_TITLE_EMOJI[1])
...
DAILY_LOGIN_REWARDS = [
    # Day 1-7 (Week 1)
    {"day": 1, "type": "xp", "amount": 25, "description": "25 XP", "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"]},
    {"day": 2, "type": "item", "rarity": "Common", "description": "Common Item", "emoji": DAILY_LOGIN_REWARD_EMOJI["item"]},
    {"day": 3, "type": "xp", "amount": 50, "description": "50 XP", "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"]},
    {"day": 4, "type": "streak_freeze", "amount": 1, "description": "Streak Freeze Token", "emoji": DAILY_LOGIN_REWARD_EMOJI["streak_freeze"]},
    {"day": 5, "type": "item", "rarity": "Uncommon", "description": "Uncommon Item", "emoji": DAILY_LOGIN_REWARD_EMOJI["item"]},
    {"day": 6, "type": "xp", "amount": 75, "description": "75 XP", "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"]},
    {"day": 7, "type": "mystery_box", "tier": "silver", "description": "Silver Mystery Box", "emoji": DAILY_LOGIN_REWARD_EMOJI["mystery_box_silver"]},

    # Day 8-14 (Week 2)
    {"day": 8, "type": "xp", "amount": 50, "description": "50 XP", "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"]},
    {"day": 9, "type": "item", "rarity": "Uncommon", "description": "Uncommon Item", "emoji": DAILY_LOGIN_REWARD_EMOJI["item"]},
    {"day": 10, "type": "multiplier", "amount": 1.5, "duration": 60, "description": "1.5x XP (1 hour)", "emoji": DAILY_LOGIN_REWARD_EMOJI["multiplier"]},
    {"day": 11, "type": "xp", "amount": 100, "description": "100 XP", "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"]},
    {"day": 12, "type": "streak_freeze", "amount": 1, "description": "Streak Freeze Token", "emoji": DAILY_LOGIN_REWARD_EMOJI["streak_freeze"]},
    {"day": 13, "type": "item", "rarity": "Rare", "description": "Rare Item", "emoji": DAILY_LOGIN_REWARD_EMOJI["item"]},
    {"day": 14, "type": "mystery_box", "tier": "gold", "description": "Gold Mystery Box", "emoji": DAILY_LOGIN_REWARD_EMOJI["mystery_box_gold"]},

    # Day 15-21 (Week 3)
    {"day": 15, "type": "xp", "amount": 75, "description": "75 XP", "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"]},
    {"day": 16, "type": "item", "rarity": "Rare", "description": "Rare Item", "emoji": DAILY_LOGIN_REWARD_EMOJI["item"]},
    {"day": 17, "type": "xp", "amount": 100, "description": "100 XP", "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"]},
    {"day": 18, "type": "streak_freeze", "amount": 2, "description": "2 Streak Freeze Tokens", "emoji": DAILY_LOGIN_REWARD_EMOJI["streak_freeze"]},
    {"day": 19, "type": "multiplier", "amount": 2.0, "duration": 60, "description": "2x XP (1 hour)", "emoji": DAILY_LOGIN_REWARD_EMOJI["multiplier"]},
    {"day": 20, "type": "item", "rarity": "Epic", "description": "Epic Item", "emoji": DAILY_LOGIN_REWARD_EMOJI["item"]},
    {"day": 21, "type": "mystery_box", "tier": "diamond", "description": "Diamond Mystery Box", "emoji": DAILY_LOGIN_REWARD_EMOJI["mystery_box_diamond"]},

    # Day 22-28 (Week 4)
    {"day": 22, "type": "xp", "amount": 100, "description": "100 XP", "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"]},
    {"day": 23, "type": "item", "rarity": "Rare", "description": "Rare Item", "emoji": DAILY_LOGIN_REWARD_EMOJI["item"]},
    {"day": 24, "type": "streak_freeze", "amount": 2, "description": "2 Streak Freeze Tokens", "emoji": DAILY_LOGIN_REWARD_EMOJI["streak_freeze"]},
    {"day": 25, "type": "xp", "amount": 150, "description": "150 XP", "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"]},
    {"day": 26, "type": "multiplier", "amount": 2.0, "duration": 120, "description": "2x XP (2 hours)", "emoji": DAILY_LOGIN_REWARD_EMOJI["multiplier"]},
    {"day": 27, "type": "item", "rarity": "Epic", "description": "Epic Item", "emoji": DAILY_LOGIN_REWARD_EMOJI["item"]},
    {"day": 28, "type": "mystery_box", "tier": "legendary", "description": "Legendary Mystery Box!", "emoji": DAILY_LOGIN_REWARD_EMOJI["mystery_box_legendary"]},
]
...
def get_daily_login_reward(login_streak: int) -> dict:
    ...
    return {
        ...
        "amount": 25,
        "description": "25 XP",
        "emoji": DAILY_LOGIN_REWARD_EMOJI["xp"],
        "login_streak": login_streak,
        "day_in_cycle": day_in_cycle,
        "cycle_number": 1,
    }
...
CELEBRATION_MESSAGES = {
    ...
    "title_unlock": [
        ...
        f"{SYMBOL_EMOJI['star']} Congratulations, {{title}}!",
    ],
    "streak_milestone": [
...
RECALC_ENTITY_NARRATIVES = {
    ...
    "underdog_008": {
        "icon_normal": DAILY_LOGIN_REWARD_EMOJI["xp"],
        "icon_exceptional": "🌟",
        ...
        "tooltip_paid": (
            "💰 PAID RECALCULATE\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            f"Granted by: {{name}} {DAILY_LOGIN_REWARD_EMOJI['xp']}\n\n"
    ...
    "underdog_006": {
        "icon_normal": SYMBOL_EMOJI["coffee"],
        "icon_exceptional": "🍬",
        ...
        "tooltip_paid": (
            "💰 PAID RECALCULATE\n"
            "━━━━━━━━━━━━━━━━━━━━━\n"
            f"Granted by: {{name}} {SYMBOL_EMOJI['coffee']}\n\n"
